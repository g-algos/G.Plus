...
public static class ElementExtension
{
    ...
    public static Parameter? GetParameter(this Element element, ElementId parameterId)
    {
        ...
        #if V2023
        int id = parameterId.IntegerValue;
        #else
        long id = parameterId.Value;
        #endif

        // --- BuiltInParameter ---
        if (id < 0)
        {
            var bip = (BuiltInParameter)id;

            return element.get_Parameter(bip)
                ?? element.Document.GetElement(element.GetTypeId())?.get_Parameter(bip);
        }

        // --- Shared / Project Parameter ---
        var paramElem = element.Document.GetElement(parameterId);
        var definition =
            (paramElem as ParameterElement)?.GetDefinition()
            ?? (paramElem as SharedParameterElement)?.GetDefinition();
