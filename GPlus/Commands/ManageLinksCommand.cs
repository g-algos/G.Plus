...
{
    private ManageLinksView window;
    ...
    
    {
        ...
        try
        {
            if (!FileHelpers.TryDocPath(ActiveCommandModel.Document, out string fileName)) return Result.Cancelled;
            ...
            viewModel.AddLinkCallback = (vm) => AddLink(vm, ActiveCommandModel.Document);

            viewModel.RemoveLink = (vm) => RemoveLink(vm, ActiveCommandModel.Document);
            viewModel.Push = (vm) => Push(vm, ActiveCommandModel.Document);
            viewModel.Pull = (vm) => Pull(vm, ActiveCommandModel.Document);
            viewModel.Sync = (vm) => Sync(vm, ActiveCommandModel.Document);
            viewModel.Merge = (vm) => Merge(vm, ActiveCommandModel.Document);
            viewModel.GetComparisonTable = Compare;
            window = new ManageLinksView(viewModel);
            _ = window.ShowDialog();
    ...
    private static (bool result, string message) Merge(Tuple<ElementId, DataTable?, DataTable?> e, Document doc)
    {
        using (Transaction transaction = new Transaction(ActiveCommandModel.Document, "Sync"))
        {
            try
            {
                ...
                options.SetFailuresPreprocessor(new HandleWarningsPreprocessor());
                ...
                if (!DataLinkSchema.TryGetSchema(view, out var link)) return (true, "");
    ...
    private static (DataTable?, DataTable?, bool, string) Compare(ElementId id)
    {
        ...
        try
        {
            ...
            if (view == null) return (null,null, true,"");

            if (DataLinkSchema.CompareTables(view, out rvtTable, out xlsTable))
                return new (rvtTable, xlsTable, true, "");
    ...
    private (bool result, string message) Sync(ElementId e, Document doc)
    {
        using (Transaction transaction = new Transaction(ActiveCommandModel.Document, "Sync"))
        {
            try
            {
                ...
                if (view == null || view is not ViewSchedule schedule) return (true,"");
                if (!DataLinkSchema.TryGetSchema(schedule, out var link)) return (true, "");
    ...
    private (bool result, string message) Pull(ElementId e, Document doc)
    {
        using (Transaction transaction = new Transaction(ActiveCommandModel.Document, "Pull"))
        {
            try
            {
                ...
                if (view == null || view is not ViewSchedule schedule) return (true,"");
                if (!DataLinkSchema.TryGetSchema(schedule, out var link)) return (true, "");
                if (!Pull(doc, schedule, link)) return (true, "");
                DataLinkSchema.Update(schedule);
                transaction.Commit();
                return (true, "");
            }
        ...
        try
        {
            ...
            foreach (var definition in definitions)
            {
                ...
                else
                {
                    ...
                    {
                        var column = unMatched.FirstOrDefault(e => e.Caption == definition.Name || e.ColumnName == definition.Name);
                        column!.ColumnName = $"{field.FieldType}:{id}";
    ...
    private (bool result, string message) Push(ElementId e, Document doc)
    {
    ...
    private (bool result, string message) RemoveLink(ElementId e, Document doc)
    {
